...
const createImagesSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [modules_openai_images_generations_create.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [modules_openai_images_generations_create.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities.push(action.payload)
        state.api.loading = "idle"
      }
    },
    [modules_openai_images_generations_create.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
