...
const scheduleCampaignsSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [modules_mailchimp_campaigns_schedule_campaign_create.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    },
    [modules_mailchimp_campaigns_schedule_campaign_create.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities.push(action.payload);
        state.api.loading = "idle";
      }
    },
    [modules_mailchimp_campaigns_schedule_campaign_create.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
