...
const paginatedLikeOnCommentListsSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [modules_disqus_comment_like_list.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    },
    [modules_disqus_comment_like_list.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = [...state.entities.filter(record => record.id !== action.payload.id), action.payload];
        state.api.loading = "idle";
      }
    },
    [modules_disqus_comment_like_list.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
