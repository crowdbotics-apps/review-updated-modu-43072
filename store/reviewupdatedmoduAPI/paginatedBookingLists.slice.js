...
export const modules_booking_booking_list = createAsyncThunk("paginatedBookingLists/modules_booking_booking_list", async payload => {
...
const paginatedBookingListsSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [modules_booking_booking_list.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    },
    [modules_booking_booking_list.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = [...state.entities.filter(record => record.id !== action.payload.id), action.payload];
        state.api.loading = "idle";
      }
    },
    [modules_booking_booking_list.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
